df <- read.csv("./Data/Processed_data/complete_dataset_2013_1999_survey.csv")
...
m_no_coast <- glm(
  presence ~ . - year - Coast_dist - Roads_dist - Urban_dist,
  data = mod_df,
  family = binomial
)

prob <- predict(m_no_coast, type = "response")

roc_obj <- pROC::roc(mod_df$presence, prob)
...
rf_model <- randomForest(
  as.factor(presence) ~ . - Coast_dist - year,
  data = mod_df,
  ntree = 500,
  importance = TRUE
)
...
rf_cv <- train(
  as.factor(presence) ~ . - year - Coast_dist - Roads_dist - Urban_dist,
  data=mod_df,
  method="rf",
  metric="ROC",
  trControl=train_control
